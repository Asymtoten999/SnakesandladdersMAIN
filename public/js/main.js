// Set initial turn to Blue
let turn = "Blue";

// Get lecSelect element
var select = document.getElementById("lecSelect");

// Set activeToken element's innerHTML to "Your opponents turn!"
document.getElementById("activeToken").innerHTML = "Your opponents turn!";

// Set Red and Blue elements' margin left and top to 0 / square number 1
...
// Get diceRoll, startBut, and restartBut elements
...
// Add click event listener to restartBut
restartBut.addEventListener("click", async (e) => {
  // If clicked is true, reload the page
  if ((clicked = true)) {
    window.location.reload();
  }
});

// Initialize stopEvent to false
let stopEvent = false;

// Initialize playerNum to 0
let playerNum = 0;

// Create socket connection
const socket = io();

// Initialize rollValue
let rollValue;

// When "player-number" event is received from socket connection:
socket.on("player-number", (num) => {
  // If num is -1:
  if (num === -1) {
    // Hide startBut and lecSelect elements
...
// When "startData" event is received from socket connection:
socket.on("startData", (start, value) => {
  // Set value of selected option in select element to value
  select.options[select.selectedIndex].value = value;
});

// When "rollData" event is received from socket connection:
socket.on("rollData", (rollData) => {
  // Set value of rollOutput element to rollData
